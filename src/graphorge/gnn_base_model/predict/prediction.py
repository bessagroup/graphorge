...
def predict(
    ...
    if model is None:
        ...
        _ = model.load_model_state(
            load_model_state=load_model_state, is_remove_posterior=False
        )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    with torch.no_grad():
        ...
        for i, pyg_graph in enumerate(
            tqdm.tqdm(data_loader, desc="> Predictions: ", disable=not is_verbose)
        ):
            ...
            if predict_directory is not None:
                save_sample_predictions(
                    predictions_dir=predict_subdir,
                    prediction_id=i,
    ...
    if is_verbose:
        ...
        print("\n> Avg. prediction loss per sample: " + loss_str)
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def make_predictions_subdir(predict_directory):
    ...
    if not os.path.exists(predict_directory):
        raise RuntimeError(
            "The model prediction directory has not been "
            "found:\n\n" + predict_directory
        )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def save_sample_predictions(
    predictions_dir, prediction_id, sample_results, file_name_pattern=None
):
    """Save model prediction results for given sample.
    ...
    if not os.path.exists(predictions_dir):
        raise RuntimeError(
            "The prediction results directory has not been "
            "found:\n\n" + predictions_dir
        )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Generate file name
    ...
    file_name = file_name_pattern.format(
        **sample_results["metadata"], prediction_id=prediction_id
    )
...
def load_sample_predictions(sample_prediction_path):
    ...
    if not os.path.isfile(sample_prediction_path):
        raise RuntimeError(
            "Sample prediction results file has not been "
...
# =============================================================================
def compute_sample_prediction_loss(
    model, loss_nature, loss_function, features_out, targets, is_normalized_loss=False
):
    """Compute loss of sample output features prediction.
...
def write_prediction_summary_file(
    ...
    summary_data["Total prediction time"] = str(
        datetime.timedelta(seconds=int(total_time_sec))
    )
    summary_data["Avg. prediction time per sample"] = str(
        datetime.timedelta(seconds=int(avg_time_sample))
    )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Write summary file
    write_summary_file(
